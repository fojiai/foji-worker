...
Key source: GOOGLE_CALENDAR_ENCRYPTION_KEY env var (base64, 32 bytes) — the
same key FojiApi and foji-ai-api use, shared here to decrypt per-agent
...
def _get_key() -> bytes:
    key_b64 = get_settings().google_calendar_encryption_key
    if not key_b64:
